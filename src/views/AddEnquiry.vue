...
const branches = computed(() => counselor.branches.items);
...
const submit = async () => {
  ...
  console.log("inside submit",formValues["education"])
  ...
  if (formValues["course_enquiries"] === 'both' && ((formValues["scourse"][0].name === null&&formValues["scourse"][0].othercourse===false)||(formValues["scourse"][0].other_course&&formValues["scourse"][0].other_course.length === 0&&formValues["scourse"][0].othercourse===true))){
    valid = false;
  ...
  for (const key in formValues) {
    if (
      key === "name" &&
      (formValues[key].length === 0 || !checkString(formValues[key]))
    ) {
      valid = false;
    ...
    if (key === "contact") {
      formValues[key].forEach((element) => {
        ...
        console.log("ele",element.code == +91 && (num.length !=10 || isNaN(num)))
    ...
    if (key === "email") {
      formValues[key].forEach((element) => {
        // if (formValues["enq_for_someone"] === false) {
          if (!/\s/g.test(element.email)&&element.email&&!validator.validate(element.email)) {
    ...
    if (key === "scourse" && ((formValues["scourse"][0].name != null)||(formValues["scourse"][0].other_course&&formValues["scourse"][0].other_course.length != 0))) {
      formValues[key].forEach((element) => {
        // || element.agreed_date.length === 0
        console.log('element inside scourse',element)
  ...
  <card-component class="mt-[18px]">
    <!-- <card-component> -->
      ...
      <field grouped class="box-border w-50 p-1">
        ...
        <control
        class="box-border w-50 p-1 border border-gray-800 mt-2 sm:mt-0 mb-2 rounded bg-gray-800 padding-space"
    ...
    md:grid-cols-3 sm:grid-cols-2 grid-cols-1 pl-2 mb-3 py-1 mt-4">
      ...
      <label class="checkbox">
        <input
          ...
          value="regular"
          class="checkbox"
          @click="((course = [{course: null, mode_class: null, branch: []}]) && (scourse = [{name: null, mode_class: null, branch: [],othercourse:false,other_course:''}]))"
        />
