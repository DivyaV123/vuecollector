...
const isOverlayVisible = computed(() => isAsideHiddenLayout.value && isAsideVisible.value)
...
onBeforeMount(async () => {
  const defaultDocumentTitle = 'QSpiders Collector'
...
<!-- <aside-menu
  ...
  >
    ...
    <div
      class="flex-1"
      :class="{'px-3':!isAsideHiddenLayout, 'flex justify-center':isAsideCompact}"
    >
      <router-link :to="`/`">
      <span :class="{'lg:hidden':isAsideCompact}">QSpiders</span> <b class="font-black">Collector</b>
      </router-link>
    </div>
  </aside-menu> -->
<aside-right />
