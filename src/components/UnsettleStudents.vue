...
import { mdiCheckboxMarkedCircle,mdiMagnify,mdiIdentifier,mdiCreditCard,mdiNumeric,mdiCloseCircleOutline,mdiAlertCircle,mdiCalendarRange,mdiCloseBox,mdiAccount,mdiPhone,mdiEmail } from '@mdi/js'
...
import CancelReceiptModal from '@/components/CancelReceiptModal.vue'
import WrongUpdateModal from '@/components/WrongUpdateModal.vue'
...
onBeforeMount(async () => {  
  ...
  await collector.get_unsettlestudents(limit,offset)
...
const confirmRefresh = async () => {
  ...
  if(searchdata.length > 0){
      collector.get_unsettlestudents(limit,offset,searchdata)     
  }
  else{
      collector.get_unsettlestudents(limit,offset)     
  }
  pageLoading.value = false;
};

const itemsPaginated = computed(() => collector.unsettlestudents.items)
// const amountvalue = computed(() => collector.unsettlestudents.amount)
// const countvalue = computed(() => collector.unsettlestudents.count)
const itemsCount = computed(() => collector.unsettlestudents.count)
const itemsData = computed(() => collector.unsettlestudents)
const branches = computed(() => collector.assigned_branches)

// const itemsCount = computed(() => collector.unsettlestudents.length)
...
const cancel_receipt = (id,name,invoiceno) => {
      propValues.id = id
      propValues.name = name
      propValues.invoiceno = invoiceno
...
const openupdatemodal = (name,invoiceno,amount,type) => {
...
const searchParams = async() => {
    searchdata=""
      ...
      pageLoading.value = true
      currentPage.value = 1
      await collector.get_unsettlestudents(limit,0,searchdata);
      pageLoading.value = false
}
...
const maxPages = ref(10);
    const numPages = computed(() => collector.unsettlestudents.total_page);
    ...
    offset = page*limit
    if (searchdata.length > 0) {
        await collector.get_unsettlestudents(limit,offset,searchdata);
    } 
    else{
      await collector.get_unsettlestudents(limit,offset)
    }
...
<template>
<cancel-receipt-modal :propValues="propValues" v-if="isCancelModalActive == true" v-model="isCancelModalActive" :title="'Cancel Invoice - ' + propValues.invoiceno" >
</cancel-receipt-modal>

<!-- <wrong-update-modal :updatedata="updatedata" v-if="isupdatemodal == true" v-model="isupdatemodal" :title="'Raise Wrong Update - ' + updatedata.invoiceno" >
</wrong-update-modal>  -->

<update-modal :updatedata="updatedata" v-if="isupdatemodal == true" v-model="isupdatemodal" :title="'Raise Wrong Update - ' + updatedata.invoiceno" @confirm="confirmRefresh" has-cancel>
</update-modal> 
...
<div class="w-full bg-[#151D2B] sticky top-[48px] z-[9] py-2">
      <!-- <span class="m-4 text-blue-400 text-xl">Amount : {{amountvalue}}</span>
      <span class="m-4 text-blue-400 text-xl">Count : {{countvalue}}</span> -->
      ...
      >
        ...
        <td data-label="Student Name">
         ...
         <td data-label="Contact Number">
          <!-- <small>  
                <div v-for="num in client.student.contact" :key="num">
        ...
        <td data-label="Comments">
          <small v-if="client.comments">    
            <span v-for="(cmt, i) in client.comments" :key="i">     
              <span v-if="cmt.comment && cmt.type && cmt.type.toLowerCase() === 'payment unsettled'">{{cmt.comment}}  <br></span>
            </span>
          </small>
        </td>
         ...
         <td data-label="Wrongly Updated"> 
          <small> 
            ...
            <!-- <jb-button
              ...
              small
              @click="cancel_receipt(client.student.id,client.student.name,client.invoiceno)"
            />  -->
            <jb-button
            v-if="client.booleans.cancelled === false"
              ...
              :icon="mdiAlertCircle"
              small
              @click="openupdatemodal(client.student_name,client.payment_details.invoiceno,client.amount,client.payment_details.type)"
            /> 
            <jb-button
            v-if="client.booleans.cancelled === true"
...
<div
  ...
  <div v-else>
    <span class="p-3 flex justify-center text-red-500 font-bold text-xl">Unsettled Payments List is empty<br/></span>
  </div>
</template>
