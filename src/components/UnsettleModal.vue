...
const confirm = async () => {
  ...
  if(valid===true){
    await collector.verify_cardpayments({"types" : "unsettle","student" : propsdata.value.payment_details.invoiceno,"comment":form.comment})               
...
<template>
  ...
  >
    ...
    >
      ...
      <div class="space-y-3">
          <span class="font-bold">Are you sure, the selected card payment of <span class="underline text-red-500">{{propsdata.amount}} Rupees</span> with authid <span class="underline text-red-500">{{propsdata.authid}}</span> & card number <span class="underline text-red-500">{{propsdata.cardno}}</span> is unsettled?</span>
        <slot />
        <field> 
        <control
